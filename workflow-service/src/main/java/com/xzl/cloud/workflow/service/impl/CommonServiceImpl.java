...
import com.xzl.cloud.workflow.dto.WfConstant;
...
public class CommonServiceImpl implements CommonService {
    ...
    //开始流程，参数放在map中传入
    public void startProcess(Map<String, Object> map, String process) {
        if (map == null)
...
//        tasks.addAll(taskService.createTaskQuery().taskCandidateUser(assignee).list());
        ...
        if (tasks.size() > 0) {
            for (Task task : tasks)
                dtos.add(new TaskDTO(task.getId(), task.getName(), task.getProcessInstanceId(), task.getCreateTime()));
        }
    ...
    //回退到指定节点
    public boolean rollBackToDest(String taskId,String destTaskkey,String advise) {
        try {
...
//        InputStream imageStream = diagramGenerator.generatePngDiagram(bpmnModel);
    ...
    public Pager getHisTasks(String assigee, String procInsId, Integer index, Integer size) {
        ...
        for (HistoricTaskInstance hisTask : list) {
            dtos.add(new TaskDTO(hisTask.getId(), hisTask.getName(), hisTask.getProcessInstanceId(), hisTask.getStartTime(), hisTask.getEndTime()));
        }
        pagenation.setList(dtos);
